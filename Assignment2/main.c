...
void mydisk_request(struct request_queue *queue)
{
	...
	{	
		  ...
		  ret=mydisk_transfer(req); 	// To move data 
		
		__blk_end_request_all(req,ret);	// suceeded as 0 and neg for failure	
...
static int __init mydisk_init(void)
{
	...
	mydisk_major = register_blkdev(mydisk_major, "dof");		//allocates dynamically
